...
void fillSegmentsMuons(DataEvtSummary_t &ev, 
  ...
  for (auto muon = muons->begin(); muon != muons->end(); muon++, whichMuon++) {
    ...
    ev.muonNsegs -> push_back(segVect->size());
	...
	if ( iSegment > (MAX_SEGS_STD-1) ) {
      ...
      ev.muon_cscsegs_chamber[whichMuon][iSegment] = id.chamber();
      ev.muon_cscsegs_nhits  [whichMuon][iSegment] = (*segmentCSC)->cscsegcand.nRecHits();
      
      if (printLevel > 2) {
	std::cout << "Endcap  = " << ev.muon_cscsegs_endcap [whichMuon][iSegment] << std::endl;
	std::cout << "Ring    = " << ev.muon_cscsegs_station[whichMuon][iSegment] << std::endl;
	std::cout << "Station = " << ev.muon_cscsegs_ring   [whichMuon][iSegment] << std::endl;
	std::cout << "Chamber = " << ev.muon_cscsegs_chamber[whichMuon][iSegment] << std::endl;
	std::cout << "nRecHits= " << ev.muon_cscsegs_nhits  [whichMuon][iSegment] << std::endl << std::endl;
      }
	...
	std::cout << "segDir.phi()   = " << muon_cscsegs_loc_dir_phi[whichMuon][iSegment] << std::endl << std::endl;
      ...
      */
      ...
      ev.muon_cscsegs_gbl_x      [whichMuon][iSegment] = globalPosition.x();
      ev.muon_cscsegs_gbl_y      [whichMuon][iSegment] = globalPosition.y();
      ev.muon_cscsegs_gbl_eta    [whichMuon][iSegment] = globalPosition.eta();
      ev.muon_cscsegs_gbl_phi    [whichMuon][iSegment] = globalPosition.phi();
      ev.muon_cscsegs_gbl_dir_eta[whichMuon][iSegment] = globalDirection.eta();
      ev.muon_cscsegs_gbl_dir_phi[whichMuon][iSegment] = globalDirection.phi();

      if (printLevel > 2) {
	std::cout << "globalPosition.x()    = " << ev.muon_cscsegs_gbl_x      [whichMuon][iSegment] << " cm"<< std::endl;
	std::cout << "globalPosition.y()    = " << ev.muon_cscsegs_gbl_y      [whichMuon][iSegment] << " cm"<< std::endl;
	std::cout << "globalPosition.eta()  = " << ev.muon_cscsegs_gbl_eta    [whichMuon][iSegment] << std::endl;
	std::cout << "globalPosition.phi()  = " << ev.muon_cscsegs_gbl_phi    [whichMuon][iSegment] << std::endl;
	std::cout << "globalDirection.eta() = " << ev.muon_cscsegs_gbl_dir_eta[whichMuon][iSegment] << std::endl;
	std::cout << "globalDirection.phi() = " << ev.muon_cscsegs_gbl_dir_phi[whichMuon][iSegment] << std::endl << std::endl;
      }
      ...
      ev.muon_cscsegs_islctable[whichMuon][iSegment] = isTriggerAble;
      ev.muon_cscsegs_ismatched[whichMuon][iSegment] = isLCTMatched;
	...
	for( ; Lct != Lctend; Lct++) {
	  ...
	  if (printLevel > 2) {
	    ...
	    match_count ++;
            ...
            if (printLevel > 2 ) cout << "Match is found. Corresponds to LCT number:" << iLCT << endl;
          }
	  
        }  // end loop CSC lcts
	
	if (whichLCT.size() == 0)
	  ev.muon_cscsegs_lctId[whichMuon][iSegment] = -999;
	
	else
	  ev.muon_cscsegs_lctId[whichMuon][iSegment] = whichLCT.front();
